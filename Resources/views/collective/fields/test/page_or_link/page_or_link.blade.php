...
<div @include('crud::inc.field_wrapper_attributes')>
    ...
    <div class="col-sm-3">
        <select id="page_or_link_select" name="{{ $field['name'] ?? 'type' }}" @include('crud::inc.field_attributes')>
            ...
            @if (count($field['options']))
                @foreach ($field['options'] as $key => $value)
                    <option value="{{ $key }}" @if (isset($field['value']) && $key == $field['value']) selected @endif>
                        {{ $value }}</option>
                @endforeach
            @endif
        </select>
            ...
            <select class="form-control" name="page_id">
                ...
                @else
                    @foreach ($active_pages as $key => $page)
                        <option value="{{ $page->id }}" @if (isset($entry) && isset($entry->page_id) && $page->id == $entry->page_id) selected @endif>
                            {{ $page->name }}</option>
                    @endforeach
                @endif
