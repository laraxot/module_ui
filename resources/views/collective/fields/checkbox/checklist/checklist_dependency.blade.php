<!-- dependencyJson -->
<div class="form-group col-xs-12 checklist_dependency" data-entity="{{ $field['field_unique_name'] }}"
    ...
    if (isset($id) && $id) {
        ...
        if (old($primary_dependency['name'])) {
            ...
            // create dependecies from relation if not from validate error
                        ...
                        <input type="checkbox" data-id="{{ $connected_entity_entry->id }}" class='primary_list'
                            ...
                            @if (
                                (isset($field['value']) &&
                                    ...
                                    (old($primary_dependency['name']) && in_array($connected_entity_entry->id, old($primary_dependency['name'])))) checked = "checked" @endif>
    ...
    <div class="row">
        ...
        @foreach ($secondary_dependency['model']::all() as $connected_entity_entry)
            <div
                class="col-sm-{{ isset($secondary_dependency['number_columns']) ? intval(12 / $secondary_dependency['number_columns']) : '4' }}">
                <div class="checkbox">
                    <label>
                        <input type="checkbox" class='secondary_list' data-id="{{ $connected_entity_entry->id }}"
                            ...
                            @if (
                                (isset($field['value']) &&
                                    is_array($field['value']) &&
                                    (in_array($connected_entity_entry->id, $field['value'][1]->pluck('id', 'id')->toArray()) ||
                                        isset($secondary_ids[$connected_entity_entry->id]))) ||
                                    (old($secondary_dependency['name']) &&
                                        in_array($connected_entity_entry->id, old($secondary_dependency['name'])))) checked = "checked"
                    ...
                    function(index, item) {
                        ...
                        thisField.find('.primary_list').change(function() {
                            ...
                            if ($(this).is(':checked')) {
                                ...
                                var inputToAdd = $('<input type="hidden" class="primary_hidden" name="' +
                                    nameInput + '[]" value="' + idCurrent + '">');
                                ...
                                $.each(dependencyJson[idCurrent], function(key, value) {
                                    ...
                                    thisField.find('input.secondary_list[value="' + value + '"]')
                                        .prop("checked", true);
                                    thisField.find('input.secondary_list[value="' + value + '"]')
                                        .prop("disabled", true);
                                    //remove hidden fields with secondary dependency if was setted
                                    var hidden = thisField.find('input.secondary_hidden[value="' +
                                        value + '"]');
                                    if (hidden)
                                        hidden.remove();
                                });
                                ...
                                var selected = [];
                                thisField.find('input.primary_hidden').each(
                                ...
                                $.each(secondary, function(index, secondaryItem) {
                                    ...
                                    $.each(selected, function(index2, selectedItem) {
                                        if (dependencyJson[selectedItem].indexOf(
                                                secondaryItem) != -1) {
                                            ok = 0;
                                        }
                                    });
                                    ...
                                    if (ok) {
                                        thisField.find('input.secondary_list[value="' +
                                            secondaryItem + '"]').prop('checked', false);
                                        thisField.find('input.secondary_list[value="' +
                                            secondaryItem + '"]').prop('disabled', false);
                                    }
                                });
                        ...
                        thisField.find('.secondary_list').click(function() {
                            ...
                            if ($(this).is(':checked')) {
                                ...
                                var inputToAdd = $('<input type="hidden" class="secondary_hidden" name="' +
                                    nameInput + '[]" value="' + idCurrent + '">');
                                ...
                                //remove hidden field with this value
                                thisField.find('input.secondary_hidden[value="' + idCurrent + '"]')
                            .remove();
                            }
                        });
