...
<div @include('crud::inc.field_wrapper_attributes')>
    ...
    <select name="{{ $field['name'] }}[]" style="width: 100%" id="select2_ajax_multiple_{{ $field['name'] }}"
        @include('crud::inc.field_attributes', ['default_class' => 'form-control']) multiple>
...
@push('crud_fields_scripts')
    <script>
        jQuery(document).ready(function($) {
            ...
            $("#select2_ajax_multiple_{{ $field['name'] }}").each(
                function(i, obj) {
            ...
            @if (isset($field['dependencies']))
                @foreach (array_wrap($field['dependencies']) as $dependency)
                    $('input[name={{ $dependency }}], select[name={{ $dependency }}], checkbox[name={{ $dependency }}], radio[name={{ $dependency }}], textarea[name={{ $dependency }}]')
                        .change(function() {
